...
public class PreordersController {
    ...
    @AutoLog("新增待接单记录")
    @PostMapping
    public Result save(@RequestBody Preorders preorders) {
	

        User dbUser = SessionUtils.getUser();
        preorders.setUserId(dbUser.getId());
		
        preordersService.save(preorders);
        return Result.success();
    }

    @AutoLog("编辑待接单记录")
    @PutMapping
    public Result update(@RequestBody Preorders preorders) {
        preordersService.updateById(preorders);
    ...
    public Result delete(@PathVariable Integer id) {
        preordersService.removeById(id);
        return Result.success();
    }
    ...
    public Result deleteBatch(@RequestBody List<Integer> ids) {
        preordersService.removeByIds(ids);
        return Result.success();
    }
    ...
    */
    ...
    public void export(HttpServletResponse response) throws Exception {
        // 从数据库查询出所有的数据
        List<Preorders> list = preordersService.list();
        // 在内存操作，写出到浏览器
        ...
        String fileName = URLEncoder.encode("Preorders信息表", "UTF-8");
    ...
    */
    ...
    public Result imp(MultipartFile file) throws Exception {
        ...
        List<Preorders> list = reader.readAll(Preorders.class);

        preordersService.saveBatch(list);
        return Result.success();
    }
