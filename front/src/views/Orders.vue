...
//催单
const chase = (id) =>{
  request.get('/front/chasorder/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
//弹出评论
state.goodids = ''
const dialogCommentFormVisible = ref(false)
const handlerCommentAdd = (id,goodids,bizUserId) =>{
  dialogCommentFormVisible.value = true
...
const saveComment = () => {
  ...
  array.forEach((e,i)=> {
    ...
    request.post('/front/comments/update', state.form).then(res => {
      if (res.code === '200') {
        dialogCommentFormVisible.value = false
      } else {
        ElMessage.error(res.msg)
        return
      }
...
<template>
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column prop="name" label="接单号"></el-table-column>
      <el-table-column label="接单明细"><template #default="scope"><el-button @click="view(scope.row.content)">查看</el-button></template></el-table-column>
      <el-table-column prop="stateRadio" label="订单状态"></el-table-column>
      <el-table-column label="接单维修工"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>
      <el-table-column label="发单用户"><template #default="scope"><span v-if="scope.row.bizUserId">{{ state.userOptions.find(v => v.id === scope.row.bizUserId) ? state.userOptions.find(v => v.id === scope.row.bizUserId).name : '' }}</span></template></el-table-column>
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('orders.edit')">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            ...
            <el-popconfirm title="您确定催单吗？" @confirm="chase(scope.row.id)" v-if="scope.row.stateRadio=='已接单'">
              <template #reference>
                <el-button type="success">催单</el-button>
              </template>
            </el-popconfirm>

            <el-button type="primary" @click="handlerCommentAdd(scope.row.id,scope.row.goodids,scope.row.user_id)" v-if="scope.row.stateRadio=='已完成'">评价</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog v-model="dialogFormVisible" title="订单信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item label-width="100" prop="name" label="接单号">
          <el-input v-model="state.form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="content" label="接单明细">
    ...
    />
        ...
        <el-form-item label-width="100" prop="stateRadio" label="订单状态">
          <el-radio-group v-model="state.form.stateRadio">
           ...
           <el-radio label="已评价">已评价</el-radio>
           <el-radio label="已取消">已取消</el-radio>
          </el-radio-group>        </el-form-item>
        <el-form-item label-width="100" prop="goodids" label="维修单编号">
    ...
    <!-- 评论窗口 -->
    <el-dialog v-model="dialogCommentFormVisible" title="评论" width="30%">
      ...
      <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogCommentFormVisible = false">取消</el-button>
