...
state.repairsOptions = []
request.get('/repairs').then(res => state.repairsOptions = res.data)
state.puserOptions = []
...
<template>
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="维修单"><template #default="scope"><span v-if="scope.row.repairsId">{{ state.repairsOptions.find(v => v.id === scope.row.repairsId) ? state.repairsOptions.find(v => v.id === scope.row.repairsId).name : '' }}</span></template></el-table-column>
    ...
    />
        ...
        <el-form-item label-width="100" prop="repairsId" label="维修单">
          <el-select clearable v-model="state.form.repairsId" placeholder="请选择"  style="width: 100%">
            <el-option v-for="item in state.repairsOptions" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
