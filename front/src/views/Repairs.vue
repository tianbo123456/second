...
const confirmDelBatch = () => {
  ...
  request.post('/repairs/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/repairs/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/repairs/export`)
}
...
<template>
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('repairs.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 添加 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('repairs.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/repairs/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('repairs.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('repairs.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="维修类型"><template #default="scope"><span v-if="scope.row.categoryId">{{ state.categoryOptions.find(v => v.id === scope.row.categoryId) ? state.categoryOptions.find(v => v.id === scope.row.categoryId).name : '' }}</span></template></el-table-column>
      <el-table-column label="发单用户"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>
      <el-table-column prop="name" label="维修标题"></el-table-column>
      <el-table-column label="详细内容"><template #default="scope"><el-button @click="view(scope.row.content)">查看</el-button></template></el-table-column>
      ...
      <el-table-column prop="stateRadio" label="状态"></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('repairs.edit') && scope.row.stateRadio=='待接单'">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('repairs.delete') && scope.row.stateRadio=='待接单'">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="维修单信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item label-width="100" prop="categoryId" label="维修类型">
        ...
        <el-form-item label-width="100" prop="name" label="维修标题">
          <el-input v-model="state.form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="content" label="详细内容">
    ...
    />
        ...
        <el-form-item label-width="100" prop="stateRadio" label="状态" v-if="user.role=='ADMIN'">
          <el-radio-group v-model="state.form.stateRadio">
           <el-radio label="待接单">待接单</el-radio>
           <el-radio label="已接单">已接单</el-radio>
